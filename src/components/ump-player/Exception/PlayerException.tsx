...
export class PlayerException extends Component {
  //constructor
  constructor(props) {
    super(props);
    //enforces 'new' instance
    if (!(this instanceof PlayerException)) {
      return new PlayerException(arguments);
    }
    var error, name,
    ...
    //access to PlayerException.prototype.name
  ...
  // but we need to raise the exception when PlayerException instance is created.
  prototype = Object.create(Error.prototype, {
    //fixes the link to the constructor (ES5)
    constructor: setDescriptor(PlayerException),
    name: setDescriptor('Player Error'),
  });

  const setDescriptor = (value: any) => {
    return {
      configurable: false,
      enumerable: false,
      writable: false,
